...
class SysUser extends Base
{
    ...
    public function index()
    {
        ...
        $this->assign('statusList',$this->model::$map_status);//所有的用户状态类型
        return $this->fetch();
    }
    ...
    public function get_user_list(){
        ...
        //id筛选
        $where = [];
        if( !empty($this->in_data['sys_user_id']) ){
            $where['sys_user_id'] = ['=',intval($this->in_data['sys_user_id'])];
        }
        //账号筛选
        if( !empty($this->in_data['keyword']) ){
            $keyword = trim($this->in_data['keyword']);
            $where['username'] = ['like',"%{$keyword}%"];
        }
        //权限组筛选
        if( !empty($this->in_data['group_id']) ){
            $where['group_id'] = ['=',intval($this->in_data['group_id'])];
        }
        //用户状态筛选
        $map_status = $this->model::$map_status;
        if( !empty($this->in_data['status']) && isset($map_status[$this->in_data['status']]) ){
            $where['status'] = $map_status[$this->in_data['status']]['value'];
        }
        ...
        if( !empty($list) ){
            foreach( $list as $k => $v ){
                ...
                $v['statusType'] = $map_status[$v['status']]['desc'];
                $list[$k] = $v;
            }
        }
    ...
    public function updateLoginCache(){
        $userInfo = $this->model->where('sys_user_id',$this->sysUserId)->find();
